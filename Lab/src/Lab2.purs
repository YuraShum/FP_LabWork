...
findIndex fn = go 0
  ...
  -- fn x = Just n якщо число завдовільняє предикат 
...
-- знаходження індекса останього елемену, для якого виконується предикат
...
unzip xy = go xy Nil Nil
  ...
  --де acc1 це значення першого списоку, acc2 значення другого списку
...
filter _ Nil = Nil
-- Якщо елемен списку задовільняє виконання предикату 
-- тоді цей елемент додано до кінцевого результату, якщо не задовільняє - 
...
tailRecursionFilter p = go Nil
  ...
  go acc (Cons x xs)
    -- якщо елемент задовільняє предикат, додаємо елемент до акумулятора
